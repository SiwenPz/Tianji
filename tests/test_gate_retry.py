...
def _review_pass(conn, controller, worker, tid):
    """双轴审核 pass(架构师确认前): 不同审核实例分别跑 spec+quality 两轴。"""
    # 注册两轴审核实例(差异化模型,满足双轴不同模型要求)
    ops.instance_register(conn, "审核规格", "claude", "deepseek-v4-flash-spec",
                          launch_cmd="python mock_worker.py")
    ops.instance_register(conn, "审核质量", "claude", "deepseek-v4-flash-quality",
                          launch_cmd="python mock_worker.py")
    did_spec = ops.dispatch_issue(conn, controller, tid, "审核规格",
    ...
    s_spec = spawn(conn, "审核规格", did_spec)
