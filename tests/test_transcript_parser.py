...
def _controller_identity(conn, monkeypatch):
    """事件写入必须使用登记时一次性发放的凭据。"""
    registered = ops.instance_register(
        conn, "总控", "claude", "deepseek-v4-flash", controller=True)
    monkeypatch.setenv("TIANJI_WORKER_ID", registered["name"])
    monkeypatch.setenv("TIANJI_SECRET", registered["secret"])
