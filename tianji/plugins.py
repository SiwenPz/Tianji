...
VIEW_SOURCES = ("task_status_counts", "instance_roster", "combo_leaderboard")
...
def combo_leaderboard(conn) -> list:
    """榜数据查询(9.5,票 25): 粒度=三元(壳+key+模型;同组合多实例取最佳)。

    主列=表现分(9.4);辅助列=结算成功率/审核拒绝率/首调延迟(9.1);
    每行带实例档案"已知坑"。冷启动: 无实测=待实测不排名;有先验声明=宣称。
    只读无按钮(操作走总控裁决通道 14.5)。
    """
    combos = {}
    for r in conn.execute(
            "SELECT i.name, i.shell, i.model, i.key_name, p.score, p.notes"
            " FROM instances i LEFT JOIN ability_profiles p"
            " ON p.instance_name=i.name WHERE i.is_active=1").fetchall():
        key = (r["shell"], r["key_name"] or "-", r["model"])
        stats = ops.post_hoc_stats(conn, r["name"])
        done = conn.execute(
            "SELECT COUNT(*) AS n FROM dispatches WHERE worker_id=?"
            " AND status='done'", (r["name"],)).fetchone()["n"]
        if done:
            status = "实测"
        elif (r["notes"] or "").strip():
            status = "宣称"
        else:
            status = "待实测"
        row = {"combo": "/".join(key), "instance": r["name"],
               "score": round(r["score"] or 0, 1),
               "success_rate": round(stats["success_rate"], 2),
               "reject_rate": round(stats["review_reject_rate"], 2),
               "first_tool_delay": round(stats["avg_first_tool_delay"], 1),
               "notes": (r["notes"] or ""), "status": status}
        old = combos.get(key)
        if old is None or row["score"] > old["score"]:
            combos[key] = row
    rows = list(combos.values())
    rows.sort(key=lambda r: (r["status"] != "实测", -r["score"]))
    return rows


def _render_view(plugin: dict, snapshot: dict, extra: dict = None) -> str:
    """视图渲染器: 输入只有只读数据(无 conn,无写账本旁路)。"""
    ...
    elif src == "combo_leaderboard":
        rows = (extra or {}).get("leaderboard") or []
        parts = []
        for r in rows:
            if r["status"] == "待实测":
                continue  # 无实测不排名(9.5 冷启动)
            pits = (r["notes"] or "无").replace("\n", ";")[:40]
            parts.append(
                f"{r['combo']} 分{r['score']} 成功率{r['success_rate']}"
                f" 拒绝率{r['reject_rate']} 坑:{pits}[{r['status']}]")
        pending = [r["combo"] for r in rows if r["status"] == "待实测"]
        body = " | ".join(parts) or "(无实测组合)"
        if pending:
            body += " | 待实测: " + ",".join(pending)
    else:
...
def render_view_blocks(conn, snapshot: dict) -> list:
    ...
    红黑榜数据源附带只读榜数据(extra 参数,渲染器仍不见 conn)。
    """
    blocks = []
    for p in list_plugins(conn):
        if p["type"] != "view" or not p.get("enabled"):
            continue
        extra = None
        if p["config"].get("source") == "combo_leaderboard":
            extra = {"leaderboard": combo_leaderboard(conn)}
        try:
            blocks.append(_render_view(p, snapshot, extra))
