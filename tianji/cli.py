...
def init(home: str = typer.Option("", "--home", help="账本根目录(默认 ~/.tianji)"),
         ...
         print_only: bool = typer.Option(False, "--print",
                                         help="只打印总控打开命令不执行")):
    """一键起步(裸跑即可): 机械扫描已装壳→选总控壳→二次确认→直接开总控会话。

    provider(key/地址/模型)不必现在给——你装的壳自带登录与默认模型,
    会话里再由总控引导你配;--shell 显式传参可跳过交互(脚本路径)。
    """
    interactive = not shell
    if interactive:
        shells = wizard.scan_shells()
        if not shells:
            typer.echo("天机是基于多 AI 助手的协作工具,但没扫到你装过任何 AI 编程助手 CLI。")
            typer.echo("先装一个(Claude Code / Codex / Kimi / AtomCode / Cline),"
                       "装好后再跑 tianji init。")
            raise typer.Exit(code=1)
        supported = [s for s in shells if s["supported"]]
        while True:
            typer.echo("")
            typer.secho("═" * 50, bold=True)
            typer.secho("  已检测到本机安装的 AI 助手:", bold=True)
            typer.secho("═" * 50, bold=True)
            typer.echo("")
            for i, s in enumerate(shells, 1):
                note = "   (已装,但天机暂不支持)" if not s["supported"] else ""
                typer.echo(f"  {i}.  {s['name']}  {s['path']}{note}")
                typer.echo("")
            if not supported:
                names = " / ".join(wizard.SHELL_ENTRY_DEFAULTS)
                typer.echo(f"你装的助手天机都还不认识——目前支持 {names},"
                           "装其中一个后重跑 tianji init。")
                raise typer.Exit(code=1)
            choice = typer.prompt(f"选一个助手当总控 (1-{len(shells)})", default="1")
            try:
                picked = shells[int(choice) - 1]
            except (ValueError, IndexError):
                typer.echo("序号不对,重来。")
                continue
            if not picked["supported"]:
                typer.echo(f"{picked['name']} 天机暂不支持,只能选支持的。")
                continue
            shell = picked["name"]
            typer.secho(f"好,总控 = {shell}。", bold=True)
            if typer.confirm("用它登录,直接开总控会话?", default=True):
                break
            typer.echo("重来,换一个。")
    r = wizard.init_bootstrap(home=home, shell=shell, model=model,
    ...
    if not interactive:
        _out(r)
        return
    typer.echo(f"账本已建: {r['home']}")
    typer.echo(f"{shell} 已设为总控,settings: {r['settings']}")
    if not r.get("provider_configured"):
        typer.echo("provider(key/地址/模型)还没配——总控会话里它自己会引导你配。")
    if shell == "claude":
        if not print_only:
            os.system("cls" if os.name == "nt" else "clear")
            typer.echo("\n\n\n")
        typer.secho("═" * 50, bold=True)
        typer.secho("  总控会话马上打开", bold=True)
        typer.echo("")
        typer.secho("  打开后,在输入框里跟它说: 你好", bold=True)
        typer.echo("")
        typer.secho("  它会自我介绍,并引导你配置 provider。", bold=True)
        typer.secho("═" * 50, bold=True)
        typer.echo("")
        if not print_only:
            input("  按回车键打开会话…")
        _launch_console(shell, print_only)
    else:
        typer.echo(f"一键开总控会话暂只支持 claude;{shell} 的会话启动方式后续版本加,"
                   "先用 tianji console(claude)或手动启动。")


def _launch_console(shell: str = "claude", print_only: bool = False) -> None:
