...
def init(home: str = typer.Option("", "--home", help="账本根目录(默认 ~/.tianji)"),
    ...
    """一键起步: 从空目录到可用总控会话(建账/总控身份/settings 一体文件/可选工人/daemon)。"""
    if not base_url:
        base_url = typer.prompt("provider base_url(如 https://api.deepseek.com/anthropic)")
    if not model:
        model = typer.prompt("模型 id", default="deepseek-v4-flash")
    if not key:
        key = typer.prompt("API key(只写本地文件,不进账本)", hide_input=True)
