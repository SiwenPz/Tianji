...
project_config_app = typer.Typer(help="默认项目目录配置")
config_app.add_typer(project_config_app, name="project")


@project_config_app.command("get")
def config_project_get():
    """读默认项目目录(18.1: 任务未指定项目工作目录时的回退来源)。"""
    row = ops.config_get(_conn(), "default_project_dir")
    _out({"default_project_dir": row["value"] if row else None})


@project_config_app.command("set")
def config_project_set(path: str,
                       request_id: str = typer.Option(None, "--request-id")):
    """默认项目目录=总控+审计(18.1)。"""
    _out(ops.config_set(_conn(), _ident(), "default_project_dir", path,
                        request_id))


@project_config_app.command("clear")
def config_project_clear(request_id: str = typer.Option(None, "--request-id")):
    """清除默认项目目录=总控+审计(回退到无默认值)。"""
    _out(ops.config_delete(_conn(), _ident(), "default_project_dir",
                           request_id))
...
def architect_review(task_id: int, verdict: str = typer.Option("reject", "--verdict",
                      help="reject(分歧驳回,默认) | confirm(分歧定夺通过)"),
                     reason: str = "",
                     request_id: str = typer.Option(None, "--request-id")):
    """架构师深审裁决(8.2): 双轴分歧时读两份审核报告定夺,裁决消息进账本。"""
    _out(ops.architect_review(_conn(), _ident(), task_id, verdict=verdict,
                              reason=reason, request_id=request_id))
...
@wizard_app.command("registry")
def wizard_registry():
    """集成注册表快照与旧条目迁移状态(13.8/票42,只读,不含密钥本体)。"""
    _out(integrations.registry_state(_conn()))


@wizard_app.command("registry-init")
def wizard_registry_init(request_id: str = typer.Option(None, "--request-id")):
    """出厂协议与常见供应商大类入库(13.8;已存在条目不覆盖)。"""
    _out(integrations.ensure_builtin_registry(
        _conn(), _ident(),
        request_id=request_id or f"cli-builtin-{ops.now()}"))


@wizard_app.command("registry-migrate")
def wizard_registry_migrate(request_id: str = typer.Option(None, "--request-id")):
    """旧 shell:/key: 条目显式迁移为集成注册表条目(13.8;幂等)。"""
    _out(integrations.migrate_legacy_entries(
        _conn(), _ident(),
        request_id=request_id or f"cli-migrate-{ops.now()}"))


@wizard_app.command("provider-add")
def wizard_provider_add(name: str, base_url: str, protocol: str,
                        key_ref: str = "",
                        request_id: str = typer.Option(None, "--request-id")):
    """显式登记可复用自定义供应商(13.8;明文 key 只允许 key_ref 引用)。"""
    _out(integrations.register_custom_provider(
        _conn(), _ident(), name, base_url, protocol, key_ref=key_ref,
        request_id=request_id or f"cli-provider-{ops.now()}"))


@wizard_app.command("credential-add")
def wizard_credential_add(name: str, provider: str, key_ref: str,
                          request_id: str = typer.Option(None, "--request-id")):
    """显式登记凭据引用条目(13.8;credential 只存 provider+key 文件引用)。"""
    _out(integrations.register_credential(
        _conn(), _ident(), name, provider, key_ref=key_ref,
        request_id=request_id or f"cli-credential-{ops.now()}"))


@wizard_app.command("discover-models")
def wizard_discover_models(
        provider: str = "", base_url: str = "", protocol: str = "",
        credential: str = "", key_ref: str = "",
        timeout: int = typer.Option(10, "--timeout")):
    """标准模型发现(票33): 多端点候选×双认证头探测;已登记供应商且有
    总控身份时结果写条目缓存(手动刷新=重跑);自定义服务给 base_url+protocol 保底。
    key 本体不进命令行——从 credential 引用/key_ref 文件现读。"""
    _out(integrations.discover_models(
        _conn(), _ident(), provider=provider, base_url=base_url,
        protocol=protocol, credential=credential, key_ref=key_ref,
        timeout=timeout))


@wizard_app.command("model-add")
def wizard_model_add(provider: str, model_id: str,
                     request_id: str = typer.Option(None, "--request-id")):
    """人工补录供应商模型并标'待实测'(13.8: 探测失败允许手录)。"""
    _out(integrations.add_provider_model(
        _conn(), _ident(), provider, model_id,
        request_id=request_id or f"cli-model-{ops.now()}"))
...
@app.command("start")
def start(home: str = typer.Option("", "--home", help="账本根目录(默认 ~/.tianji)"),
          no_browser: bool = typer.Option(False, "--no-browser",
                                          help="不自动打开浏览器")):
    """一键起步(裸跑即可): 建账+总控身份+起 daemon(monitor+web),
    浏览器自动打开配置页。

    启动机械引导一次工作目录(2026-08-24 用户裁决,推翻零问答): 回车=当前
    目录;该目录写默认项目目录并作为总控会话 cwd(总控直接在你的项目里干活)。
    选总控助手/配实例/定角色全在 web 配置页纯点选;重跑幂等
    (secret 不轮换,照旧从 ctrl-secret.txt 读回)。
    """
    import webbrowser
    work_dir = input(f"工作目录(回车=当前目录 {os.getcwd()}): ").strip()
    if not work_dir:
        work_dir = os.getcwd()
    r = wizard.init_bootstrap(home=home, shell="未配置", start_daemon=True,
                              work_dir=work_dir)
    url = f"http://127.0.0.1:{r['daemon']['web_port']}/setup"
    typer.echo(f"  账本: {r['home']}")
    for s in r["steps"]:
        typer.echo(f"  {s}")
    if no_browser:
        typer.echo(f"  配置页: {url}(手动打开)")
    else:
        webbrowser.open(url)
        typer.echo(f"  配置页: {url}(已自动打开,没开就手动复制)")




def _launch_console(shell: str = "claude", print_only: bool = False,
                    greeting: str = "你好,天机") -> None:
    """开总控会话(身份在 settings 一体文件,零手工环境变量)。

    greeting=开场白,作为首条消息直接带进会话(位置参数),打开即开始引导,
    用户不用再手动打招呼。措辞与入阵页分工: 入阵页是"吾乃天机 · 剑来",
    注入会话的首条消息是"你好,天机"(2026-08-21 用户定)。"""
    ...
    if not settings.exists():
        ...
        return
    exe = _sh.which(shell) or shell
    cmd = [exe, "--settings", str(settings)]
    # 总控角色自述: settings 文件里的 appendSystemPrompt 键实测不被 --settings
    # 加载(2026-08-20 模拟实证),用 CLI 参数显式注入,话术仍以文件为单一来源
    role = _json.loads(settings.read_text(encoding="utf-8")
                       ).get("appendSystemPrompt")
    if role:
        cmd = [exe, "--append-system-prompt", role, "--settings", str(settings)]
    if greeting:
        cmd.append(greeting)
    if print_only:
        _out({"cmd": " ".join(cmd)})
...
def console(print_only: bool = typer.Option(False, "--print",
    ...
    _launch_console("claude", print_only)
