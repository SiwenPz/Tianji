...
- 账号池轮盘：多个 key 攒成池，轮盘选员、熔断恢复、协议自动转换，老出错的 key 自动下场歇会儿，池子整体对外就是一个稳住的身份
...
也得说实话，离"什么牌都能打"的理想还有距离：总控本身吃 token 不少，靠缓存命中找补的路子还在琢磨；低档模型和时好时坏的 key 闹出的频繁返工，账号池轮盘这一版先把"老出错的就下场歇会儿"做实了——池子熔断、恢复、按实例收口的耗尽信号都上了，用量均衡和池间互备还在等真实数据说话。骨架立住了，肉还在长。
